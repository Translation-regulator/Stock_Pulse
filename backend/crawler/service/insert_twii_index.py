...
def fetch_summary_by_date(date):
    ...
    print(f"{date} volume: {volume}, trade_count: {trade_count}, change: {change_point}")
    return volume, trade_count, change_point
...
def fetch_twii_by_month(year, month):
    ...
    for row in rows[2:]:
        ...
        try:
            ...
            volume, trade_count, change_point = fetch_summary_by_date(ad_date)
    ...
    for item in data:
        try:
            cursor.execute("""
                ...
                (date, open, high, low, close, volume, trade_count, change_point)
